...
function filterByValue(arr, key){
  return arr.filter(element => element[key]);
}
...
function find(arr, searchValue){
  return arr.filter(element => element === searchValue)[0];
}
...
function findInObj(arr, key, searchValue){
  return arr.filter(element => element[key] === searchValue)[0];
}
...
function removeVowels(str){
  ...
  const vowels = 'aeiou';
  return arr.filter(element => vowels.indexOf(element) === -1).join('');
}
...
function doubleOddNumbers(arr){
  ...
  return filteredArr.map(element => element * 2);
}
